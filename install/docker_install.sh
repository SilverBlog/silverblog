...
cat << EOF >manage.sh
#!/usr/bin/env bash
docker exec -it silverblog_control python3 manage.py \$@
EOF
chmod +x manage.sh
echo "Before you start SilverBlog for the first time, run the following command to initialize the configuration:"
echo "docker run -it --rm -v \$(pwd):/home/silverblog --name=\"silverblog_manage\" silverblog/silverblog python3 manage.py setting"